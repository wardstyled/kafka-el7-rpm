...
install -p -D -m 644 config/server.properties $RPM_BUILD_ROOT%{_prefix}/%{name}/config/
sed -i "s:^log.dirs=.*:log.dirs=%{_data_dir}:" $RPM_BUILD_ROOT%{_prefix}/%{name}/config/server.properties
